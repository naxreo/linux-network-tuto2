...
웹 페이지로 하나의 큰 맵과 같은 형태의 블록다이어그램을 만들고 키보드나 마우스를 통해서 각 파트의 세부 사항 부분을 확대해서 움직이는 형태로 리눅스 네트워크 흐름도를 만들기 위해서는 어떻게 구현할 수 있을까?
...
1️⃣ 각 함수 → 커널 소스 파일 경로 매핑
2️⃣ ftrace / perf 기준 실제 호출 순서 시각화
3️⃣ Cytoscape에서 함수 단위 노드 자동 생성 구조

어디까지 내려가고 싶은지 말씀해 주시면, 그 깊이까지 바로 이어서 정리해 드리겠습니다.

# 요구 사항

- 로컬 환경에서 동작될 수 있도록 위 내용을 프로그래밍해주세요.
- 리눅스 네트워크 계위나 계층이 다른 경우 다른 색상으로 구분해주세요.
